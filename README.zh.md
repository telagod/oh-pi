...
我们不再把产品核心定义为：
- 主题、预设、技能、快捷配置的集合
- “oh-my-zsh for pi” 式 convenience layer

我们现在把产品核心定义为：
- **Ant Colony for Pi**
- **一个为复杂编码任务设计的多智能体协作插件**
- **让 pi 具备侦察、拆解、并行执行、审查与恢复能力**

### 它解决什么问题

当任务具备这些特征时，单 agent 往往开始吃力：
- 涉及 **3 个以上文件**
- 需要 **跨模块理解与重构**
- 可以拆成 **多个并行子任务**
- 需要 **执行后统一审查与修复闭环**

Ant Colony 的目标不是替代 pi，而是让 pi 在这类任务上拥有一个更强的执行层。

## 30 秒开始

```bash
npx oh-pi             # 兼容期内的 legacy/bootstrap 安装路径
npx pi-agent-colony   # canonical 安装路径
pi                    # 在 pi 中使用蚁群能力
```

当前阶段，`oh-pi` 仍作为兼容桥接名称保留，并继续负责把能力写入 `~/.pi/agent/`。
后续默认名称会逐步切到 `pi-agent-colony`，产品公开名称则保持 **Agent Colony / Agent Colony for Pi**。
...
```text
“把认证从 session 重构为 JWT，并补齐测试与回归检查”
```

### Ant Colony 会这样工作

```text
1. scout 侦察代码库，定位边界与依赖
2. planner 生成任务池与执行顺序
3. workers 并行修改不同文件/模块
4. reviewer 统一审查变更，必要时退回复修
5. 汇总最终结果并注入主对话
```

这才是本项目真正的差异化：**复杂任务吞吐、任务分解能力、以及执行可靠性**。

- [`docs/DEMO-SCRIPT.zh.md`](./docs/DEMO-SCRIPT.zh.md) — 2 分钟演示脚本
- [`ROADMAP.md`](./ROADMAP.md) — 新定位下的里程碑
- [`DECISIONS.md`](./DECISIONS.md) — 定位调整与架构取舍
- [`docs/PRODUCT.md`](./docs/PRODUCT.md) — 产品边界、适用场景与非目标
- [`docs/ARCHITECTURE-REFACTOR.md`](./docs/ARCHITECTURE-REFACTOR.md) — 插件边界与重构方向（英文）
