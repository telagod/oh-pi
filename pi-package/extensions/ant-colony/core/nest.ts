...
import { JsonlPheromoneStore, type PheromoneStore } from "./pheromone-store.js";
...
export class Nest {
  ...
  constructor(private cwd: string, private colonyId: string, pheromoneStore?: PheromoneStore) {
    ...
    this.pheromoneStore = pheromoneStore ?? new JsonlPheromoneStore(this.pheromoneFile);
  }
  ...
  dropPheromone(p: Pheromone): void {
    this.pheromoneStore.append(p);
    this.pheromoneIndexDirty = true;
  }

  getAllPheromones(): Pheromone[] {
    const prevCache = this.pheromoneCache;
    const nextCache = this.pheromoneStore.listActive();
    this.pheromoneCache = nextCache;

    if (this.pheromoneIndexDirty || prevCache !== nextCache || prevCache.length !== nextCache.length) {
  ...
  destroy(): void {
    try { this.pheromoneStore.close?.(); } catch { /* ignore */ }
